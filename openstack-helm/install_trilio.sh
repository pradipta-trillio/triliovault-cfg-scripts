...
STEPS=(
  ...
  db_users     # align pre-existing MariaDB users with the generated passwords
...
# helm-toolkit's db-init does CREATE USER IF NOT EXISTS ... IDENTIFIED BY, which
# is a no-op for a user that already exists — it does NOT update the password.
# It then verifies by connecting as that user. So on a cluster where a previous
# T4O install left the dmapi / workloadmgr MariaDB users behind (helm delete
# does not drop them), a fresh triliovault_passwords.yaml gives you a db-init
# job that cannot log in as its own user, and the pod crash-loops with
# "Could not connect to database as user".
#
# ALTER USER closes the gap without touching any data.
step_db_users() {
  [[ $DRY_RUN -eq 1 ]] && { step_skip "db users (dry run)"; return 0; }
  [[ -f "$VO_DIR/triliovault_passwords.yaml" ]] || { step_skip "no passwords file yet"; return 0; }
  OS_NAMESPACE="$OS_NAMESPACE" run_util reset_db_passwords.sh || return 1
  step_ok "MariaDB users aligned with triliovault_passwords.yaml"
}
...
step_install() {
  ...
  while read -r f; do
    ...
    elif [[ $DRY_RUN -eq 1 && -f "$VO_DIR/$f.example" ]]; then
      # ONLY under --dry-run. The .example files carry <GENERATED> placeholders;
      # feeding them to a real install would deploy placeholder credentials and
      # the db-init job would then fail to connect as its own user.
      args+=(--values="$VO_DIR/$f.example"); substituted+=("$f")
    else
      err "missing values file: $VO_DIR/$f"
      err "It is generated by an earlier step. Re-run that step, e.g."
      err "    bash ./install_trilio.sh --only admin_creds"
      return 1
    fi
  done < <(values_list)
  (( ${#substituted[@]} )) && warn "DRY RUN ONLY: rendering with .example placeholders for: ${substituted[*]}"
